 ...
 */
public class ShutdownActive extends DRTTestCase {
    ...
    public ShutdownActive(String testCaseName) {
        super(testCaseName);
        serverStatusController = ServerStatusController.getInstance();
        executionController = ExecutionController.getInstance();
    }

    public ShutdownActive(TestCaseElement testCaseConfig) {
        super(testCaseConfig);
        serverStatusController = ServerStatusController.getInstance();
        executionController = ExecutionController.getInstance();
    }

    public ShutdownActive(TestCaseElement testCaseConfig, ServerStatusController serverStatusController) {
    ...
    public static ArrayList getOrder() {
        ...
        methodsOrder.add("testShutDownActive");
     ...
     */
    public void testShutDownActive() {
        DRTTestLogger.logMethodOrder(DRTTestConstants.getOrderMessage("testShutDownActive", "ShutdownActive:mode=" + testCaseProperties.getProperty("MODE")));
        ...
        try {
            ...
            System.out.println("Machine 1 status before shutting down the active server = " + oldStatusMachine1);
            System.out.println("Machine 3 status before shutting down the active server = " + oldStatusMachine3);			
            ...
            if (m1IsActive) {
                ...
                if (!newStatusMachine3.equalsIgnoreCase("standalone")) {
                    Assert.fail("TestCase testShutDownActive of class " + ShutdownActive.class.toString() + " in mode = " + testCaseProperties.getProperty("MODE") + " failed !!! Reason: The Passive server didn't go to Standalone state after shutting down the Active Server");
                ...
                if (!newStatusMachine1.equalsIgnoreCase("passive")) {
                    Assert.fail("TestCase testShutDownActive of class " + ShutdownActive.class.toString() + " in mode = " + testCaseProperties.getProperty("MODE") + " failed !!! Reason: The server which was shut down was started again but it didn't switch to passive");
                }
            } else {
                ...
                newStatusMachine1 = serverStatusController.getServerStatus(machineElement1.getAddress(), primaryRMIPort);
                System.out.println("Machine 1 status after shutting down Active server at machine 3 = " + newStatusMachine1);
            ...
            DRTTestLogger.log(Level.SEVERE, "test case: testShutDownActive OF class " + ShutdownActive.class.toString() + " in mode = " + testCaseProperties.getProperty("MODE") + " failed !!! ", e);
            e.printStackTrace();
            Assert.fail("TestCase testShutDownActive OF class " + ShutdownActive.class.toString() + " in mode = " + testCaseProperties.getProperty("MODE") + " failed !!!" + e.getMessage());
        }
        DRTTestLogger.log(DRTTestConstants.getFinishMessage("testShutDownActive", "ShutdownActive:mode=" + testCaseProperties.getProperty("MODE")));
        System.out.println("-------------------");

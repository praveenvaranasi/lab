...
//                }
    ...
    private List getPeerServerNames(Enumeration servers) {
        ...
        while (servers.hasMoreElements()) {
            ...
            if (server.getMode().equalsIgnoreCase(TestEnvironmentConstants.AGS))
                peerServers.add(server.getServerName());
        }
        System.out.println("PeerServers: "+peerServers);
        return peerServers;

    }

    private String getUrl(String hostName) {  //to get the URL of the active FES port num
    ...
    private void startServers(TestSuiteElement testSuite) {
        ...
        for (Enumeration servers = peerServersList.elements(); servers.hasMoreElements();) {
            TCServerElement tcServer = (TCServerElement) servers.nextElement();
            startServer(tcServer, TestEnvironmentConstants.AMS);
        }
        for (Enumeration servers = peerServersList.elements(); servers.hasMoreElements();) {
            TCServerElement tcServer = (TCServerElement) servers.nextElement();
            startServer(tcServer, TestEnvironmentConstants.AGS);
        }
    }

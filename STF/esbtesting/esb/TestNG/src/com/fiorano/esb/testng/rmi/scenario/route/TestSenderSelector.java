...
*/
public class TestSenderSelector extends AbstractTestNG{
    ...
    @BeforeClass(groups = "SenderSelectorTest", alwaysRun = true)
    public void startSetUp(){
        initializeProperties("scenario" + fsc + "Route" + fsc + "SenderSelector" + fsc + "tests.properties");
        resourceFilePath = testResourcesHome + fsc + "tests" +  fsc + "scenario" + fsc + "Route" + fsc + "SenderSelector";
    ...
    @Test(groups = "SenderSelectorTest", alwaysRun = true, description = "trying to import an application.")
    public void testImportApplication() {
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testImportApplication", "TestSenderSelector"));
            ...
            Logger.Log(Level.INFO, Logger.getFinishMessage("testImportApplication", "TestSenderSelector"));
        }
        catch (Exception ex) {
    ...
    @Test(groups = "SenderSelectorTest", alwaysRun = true, dependsOnMethods = "testImportApplication", description = "Runs the application")
    public void testRunApplication() {
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testRunApplication", "TestSenderSelector"));
            System.out.println("Started the Execution of the TestCase::" + getName());
            startApplication(m_appGUID, m_version);
            Thread.sleep(5000);
            if (!m_fioranoApplicationController.isApplicationRunning(m_appGUID, m_version))
                throw new Exception("The Application is not started.");
            Logger.Log(Level.INFO, Logger.getFinishMessage("testRunApplication", "TestSenderSelector"));
        }
        catch (Exception ex) {
            System.err.println("Exception in the Execution of test case::" + getName());
            ex.printStackTrace();
    ...
    @Test(groups = "SenderSelectorTest", alwaysRun = true, dependsOnMethods = "testRunApplication", description = "sets the output port of chat1 to topic and input port of chat2 and chat3 to queue and tests the change")
    public void testSenderSelectorPositive() {
        boolean check = false;
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testSenderSelectorPositive", "SenderSelectorTest"));
            System.out.println("Started the Execution of the TestCase::" + getName());
            ...
            Logger.Log(Level.INFO, Logger.getFinishMessage("testSenderSelectorPositive", "SenderSelectorTest"));
        }
        catch (Exception ex) {
            System.out.println("Exception in the Execution of test case::" + ex.getMessage());
            System.err.println("Exception in the Execution of test case::" + getName());
            ex.printStackTrace();
            Logger.Log(Level.SEVERE, Logger.getErrMessage("testSenderSelectorPositive", "SenderSelectorTest"), ex);
            Assert.assertTrue(false, "TestCase Failed because of " + ex.getMessage());

        }
    }

    @Test(groups = "SenderSelectorTest", alwaysRun = true, dependsOnMethods = "testSenderSelectorPositive", description = "sets the output port of chat1 to Queue and input port of chat2 and chat3 to topic and tests the change")
    public void testSenderSelectorNegative() {
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testSenderSelectorNegative", "SenderSelectorTest"));
            System.out.println("Started the Execution of the TestCase::" + getName());
            File file = new File(m_outFile);
            if (file.exists())
                file.delete();
            eventProcessHandle.sendMessageOnTopic("chat1", "OUT_PORT", new File(m_testFile1));    //todo exception here
            Thread.sleep(20000);
            if (!file.exists())
            ...
            Logger.Log(Level.INFO, Logger.getFinishMessage("testSenderSelectorNegative", "SenderSelectorTest"));
        }
        catch (Exception ex) {
            System.out.println("Exception in the Execution of test case::" + ex.getMessage());
            System.err.println("Exception in the Execution of test case::" + getName());
            ex.printStackTrace();
            Logger.Log(Level.SEVERE, Logger.getErrMessage("testSenderSelectorNegative", "SenderSelectorTest"), ex);
            Assert.assertTrue(false, "TestCase Failed because of " + ex.getMessage());

        }
    }

    @Test(groups = "SenderSelectorTest", alwaysRun = true, dependsOnMethods = "testSenderSelectorNegative", description = "Stops the running application.")
    public void testKillApplication() {
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testKillApplication", "TestSenderSelector"));
            System.out.println("Started the Execution of the TestCase::" + getName());
            stopApplication(m_appGUID);
            if (m_fioranoApplicationController.isApplicationRunning(m_appGUID))
                throw new Exception("Application not killed");
            Logger.Log(Level.INFO, Logger.getFinishMessage("testKillApplication", "TestSenderSelector"));
    ...
    @Test(groups = "SenderSelectorTest", alwaysRun = true, dependsOnMethods = "testKillApplication", description = "Deletes the stopped application.")
    public void testDeleteApplication() {
        try {
            Logger.LogMethodOrder(Logger.getOrderMessage("testDeleteApplication", "TestSenderSelector"));
            System.out.println("Started the Execution of the TestCase::" + getName());
            m_fioranoApplicationController.deleteApplication(m_appGUID, m_version);
            Logger.Log(Level.INFO, Logger.getFinishMessage("testDeleteApplication", "TestSenderSelector"));
        }
        catch (Exception ex) {
            System.err.println("Exception in the Execution of test case::" + getName());
            ex.printStackTrace();

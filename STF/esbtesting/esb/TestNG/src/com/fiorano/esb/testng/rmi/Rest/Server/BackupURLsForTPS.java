...
public class BackupURLsForTPS {
    ...
    public void execute() throws URISyntaxException, JSONException, IOException, InterruptedException {
        ...
        //http://localhost:1980/api/fes/servers/peer/backupURLsForPeer/fps?api_key=YpncFMeSadRtxN-iSgCALtvY
        String AppRunUri="http://localhost:1980/api/fes/servers/peer/backupURLsForPeer/"+serverName+"?api_key="+key;
        output = restCall1(AppRunUri);
        System.out.println(output);

        //**Invalid server name
        serverName="ssdftr";
        AppRunUri="http://localhost:1980/api/fes/servers/peer/backupURLsForPeer/"+serverName+"?api_key="+key;
        output = restCall1(AppRunUri);
        System.out.println(output);
